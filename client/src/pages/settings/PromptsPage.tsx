...
import { useAdminPrompts, useUpdatePrompt, useDeletePrompt } from '@/hooks/useAdmin';
...
export default function PromptsPage() {
  ...
  return (
    <div className="flex flex-col gap-6">
      <h1 className="text-2xl font-bold tracking-tight">System Prompts</h1>
      ...
      {isLoading ? (
        ...
        <EmptyState title="No prompts" description="No system prompt templates configured yet." />
      ) : (
        <div className="space-y-3">
          {prompts.map((prompt) => (
            <Card key={prompt.id}>
        ...
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button variant="destructive" onClick={handleDelete} disabled={deletePrompt.isPending}>
              {deletePrompt.isPending ? <Loader2 className="mr-2 size-4 animate-spin" /> : 'Delete'}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>

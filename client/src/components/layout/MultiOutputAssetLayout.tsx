/**
 * MultiOutputAssetLayout — detail layout for assets with multiple output types (Actors).
 *
 * Structure:
 ...
 *   - Tabs: Overview (metadata), Outputs (collapsible sections per output type), Properties (taxonomy)
 *
...
export interface MultiOutputAssetLayoutProps {
  /** Breadcrumb library name (e.g. "Actors") */
  libraryLabel: string;
  /** Breadcrumb library path (e.g. "/actors") */
  libraryPath: string;
  /** Asset name shown in header + breadcrumb */
  name: string;
  /** Type badge text (e.g. "Actor") */
  typeLabel: string;
  /** Optional status badge (generation or marketplace status) */
  statusBadge?: ReactNode;
  /** Action toolbar buttons (edit, regenerate, duplicate, submit, etc.) */
  actions: ReactNode;
  /** Main image for the sidebar / stacked mobile view (e.g. headshot) */
  image: ReactNode;
  /** Overview tab content (metadata, taxonomy summary) */
  overviewContent: ReactNode;
  /** Outputs tab content (collapsible sections per output type) */
...
export default function MultiOutputAssetLayout({
  libraryLabel,
  libraryPath,
  name,
  typeLabel,

...
| key_hash     | VARCHAR      | Not Null, Unique                     | Hashed API key (display prefix: cs*live*...).                  |
...
**Reproducibility:** `generation_params` stores the complete JSON body sent to fal.ai (model, seed, resolution, steps, guidance*scale, sampler, num_outputs, prompt, and any other API parameters). This allows exact replay of the generation call. `reference_images` stores uploaded input images with naming convention `ref*{asset*id}*{version}\_{short_uuid}.png`for traceability.`source_asset_outputs` links to existing assets used as input (e.g., character sheet uses headshot + look outputs).

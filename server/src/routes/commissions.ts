...
} from '../validation/commission-schemas.js';
...
router.post('/', requireSession, requireWorkspace, async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(500)
      .json({ error: { code: 'INTERNAL_ERROR', message: 'Failed to create commission' } });
  }
});
...
router.get('/', requireSession, requireWorkspace, async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(500)
      .json({ error: { code: 'INTERNAL_ERROR', message: 'Failed to list commissions' } });
  }
});
...
router.get('/:id', requireSession, requireWorkspace, async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(500)
      .json({ error: { code: 'INTERNAL_ERROR', message: 'Failed to get commission' } });
  }
});
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!adminBypass) {
        res
          .status(403)
          .json({ error: { code: 'FORBIDDEN', message: 'Only admins can assign commissions' } });
        return;
      }
      ...
      res
        .status(500)
        .json({ error: { code: 'INTERNAL_ERROR', message: 'Failed to assign commission' } });
    }
  },
);
...
router.patch(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      res
        .status(500)
        .json({ error: { code: 'INTERNAL_ERROR', message: 'Failed to update commission status' } });
    }
  },
);
